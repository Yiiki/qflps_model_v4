...
args_pts = [2,2,2,2,args_config_func(1:3)];
sigval = 0.1;
sigs_num = 1;

lovs=1e-1;
% upvs=1e1;
upvs=args_config_func(4);

args_len = sum(args_pts);

% generate randv initial batch
% loup = [...
% 1e-5,   3e-2;  % ue
% -1,      6;      % vthn
% 1e-5,   3e-2; % uh
...
rand_intv_set=rand(args_len,samp_num).*(upv_here-lov_here)+lov_here;

% write rand initial into func
func_name = 'rand_data_func';
randfh=str2func(func_name);

fileID=fopen([func_name,'.m'],'w');
